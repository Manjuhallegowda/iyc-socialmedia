...
var hashPassword = /* @__PURE__ */ __name(
  async (password, saltBase64, iterations = 15e4) => {
...
var createCrudEndpoints = /* @__PURE__ */ __name(
  (router, tableName, options = {}) => {
    ...
    router.get(basePath, async (c) => {
      try {
        const res = await c.env.DB.prepare(`SELECT * FROM ${tableName}`).all();
...
createCrudEndpoints(api, 'executive_leaders', executiveLeaderTransforms);
var stateLeadersRouter = new Hono2();
stateLeadersRouter.get('/', async (c) => {
  try {
    const leadersRes = await c.env.DB.prepare(
      'SELECT * FROM state_leaders'
    ).all();
    const leaders = (leadersRes.results || leadersRes).map((l) => ({
      ...l,
      socialMedia: JSON.parse(l.socialMedia || '{}'),
      activities: [],
      milestones: [],
    }));
    ...
    const milestonesRes = await c.env.DB.prepare(
...
stateLeadersRouter.get('/:id', async (c) => {
  const id = c.req.param('id');
  try {
    const leaderRes = await c.env.DB.prepare(
      'SELECT * FROM state_leaders WHERE id = ?'
    )
      .bind(id)
      .first();
    ...
    const leader = {
      ...leaderRes,
      socialMedia: JSON.parse(leaderRes.socialMedia || '{}'),
    };
    const activitiesRes = await c.env.DB.prepare(
      'SELECT * FROM activities WHERE leaderId = ?'
    )
      .bind(id)
      .all();
    const activities = activitiesRes.results || activitiesRes;
    const milestonesRes = await c.env.DB.prepare(
      'SELECT * FROM milestones WHERE leaderId = ?'
    )
      .bind(id)
      .all();
    const milestones = milestonesRes.results || milestonesRes;
