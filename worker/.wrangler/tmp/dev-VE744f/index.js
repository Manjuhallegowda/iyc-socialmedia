...
// .wrangler/tmp/bundle-KIn8rP/checked-fetch.js
...
var generateSaltBase64 = /* @__PURE__ */ __name((len = 16) => {
...
var createCrudEndpoints = /* @__PURE__ */ __name((router, tableName, options = {}) => {
  const basePath = `/${tableName}`;
  const transformAfterFetch = /* @__PURE__ */ __name((item) => {
    return options.afterFetch ? options.afterFetch(item) : item;
  }, "transformAfterFetch");
  const transformBeforeSave = /* @__PURE__ */ __name((item) => {
...
var activityTransforms = {
  beforeSave: /* @__PURE__ */ __name((activity) => ({
    ...activity,
    stats: JSON.stringify(activity.stats || [])
  }), "beforeSave"),
  afterFetch: /* @__PURE__ */ __name((activity) => ({
    ...activity,
...
var executiveLeaderTransforms = {
  beforeSave: /* @__PURE__ */ __name((executiveLeader) => ({
    ...executiveLeader,
    socialMedia: JSON.stringify(executiveLeader.socialMedia || {})
  }), "beforeSave"),
  afterFetch: /* @__PURE__ */ __name((executiveLeader) => ({
    ...executiveLeader,
...
createCrudEndpoints(api, "gallery_items");
createCrudEndpoints(
  api,
  "executive_leaders",
  executiveLeaderTransforms
);
var stateLeadersRouter = new Hono2();
stateLeadersRouter.get("/", async (c) => {
  try {
    const leadersRes = await c.env.DB.prepare("SELECT * FROM state_leaders").all();
    const leaders = (leadersRes.results || leadersRes).map((l) => ({ ...l, socialMedia: JSON.parse(l.socialMedia || "{}"), activities: [], milestones: [] }));
    const activitiesRes = await c.env.DB.prepare("SELECT * FROM activities").all();
    const activities = activitiesRes.results || activitiesRes;
    const milestonesRes = await c.env.DB.prepare("SELECT * FROM milestones").all();
    const milestones = milestonesRes.results || milestonesRes;
    const leaderMap = new Map(leaders.map((l) => [l.id, l]));
    activities.forEach((activity) => {
      if (activity.leaderId && leaderMap.has(activity.leaderId)) {
        leaderMap.get(activity.leaderId).activities.push(activity);
      }
    });
    milestones.forEach((milestone) => {
      if (milestone.leaderId && leaderMap.has(milestone.leaderId)) {
        leaderMap.get(milestone.leaderId).milestones.push(milestone);
      }
    });
    return c.json(Array.from(leaderMap.values()));
  } catch (e) {
    console.error("Error fetching state leaders:", e);
    return c.json({ error: e.message }, 500);
  }
});
stateLeadersRouter.get("/:id", async (c) => {
  const id = c.req.param("id");
  try {
    const leaderRes = await c.env.DB.prepare("SELECT * FROM state_leaders WHERE id = ?").bind(id).first();
    if (!leaderRes) return c.json({ error: "Not Found" }, 404);
    const leader = { ...leaderRes, socialMedia: JSON.parse(leaderRes.socialMedia || "{}") };
    const activitiesRes = await c.env.DB.prepare("SELECT * FROM activities WHERE leaderId = ?").bind(id).all();
    const activities = activitiesRes.results || activitiesRes;
    const milestonesRes = await c.env.DB.prepare("SELECT * FROM milestones WHERE leaderId = ?").bind(id).all();
    const milestones = milestonesRes.results || milestonesRes;
    return c.json({ ...leader, activities, milestones });
  } catch (e) {
    console.error(`Error fetching state leader ${id}:`, e);
    return c.json({ error: e.message }, 500);
  }
});
stateLeadersRouter.post("/", jwtMiddleware, async (c) => {
  try {
    const body = await c.req.json();
    const { activities, milestones, socialMedia, ...leaderData } = body;
    const id = generateUUID();
    const batch = [];
    batch.push(c.env.DB.prepare(
      "INSERT INTO state_leaders (id, name, designation, state, bio, imageUrl, socialMedia) VALUES (?, ?, ?, ?, ?, ?, ?)"
    ).bind(id, leaderData.name, leaderData.designation, leaderData.state, leaderData.bio, leaderData.imageUrl, JSON.stringify(socialMedia || {})));
    (activities || []).forEach((activity) => {
      batch.push(c.env.DB.prepare(
        "INSERT INTO activities (id, title, date, description, type, leaderId) VALUES (?, ?, ?, ?, ?, ?)"
      ).bind(generateUUID(), activity.title, activity.date, activity.description, "Event", id));
    });
    (milestones || []).forEach((milestone) => {
      batch.push(c.env.DB.prepare(
        "INSERT INTO milestones (id, date, description, leaderId) VALUES (?, ?, ?, ?)"
      ).bind(generateUUID(), milestone.date, milestone.description, id));
    });
    await c.env.DB.batch(batch);
    return c.json({ ...body, id }, 201);
  } catch (e) {
    console.error("Error creating state leader:", e);
    return c.json({ error: e.message }, 500);
  }
});
stateLeadersRouter.put("/:id", jwtMiddleware, async (c) => {
  const id = c.req.param("id");
  try {
    const body = await c.req.json();
    if (id !== body.id) return c.json({ error: "ID mismatch" }, 400);
    const { activities, milestones, socialMedia, ...leaderData } = body;
    const batch = [];
    batch.push(c.env.DB.prepare(
      "UPDATE state_leaders SET name = ?, designation = ?, state = ?, bio = ?, imageUrl = ?, socialMedia = ? WHERE id = ?"
    ).bind(leaderData.name, leaderData.designation, leaderData.state, leaderData.bio, leaderData.imageUrl, JSON.stringify(socialMedia || {}), id));
    batch.push(c.env.DB.prepare("DELETE FROM activities WHERE leaderId = ?").bind(id));
    batch.push(c.env.DB.prepare("DELETE FROM milestones WHERE leaderId = ?").bind(id));
    (activities || []).forEach((activity) => {
      batch.push(c.env.DB.prepare(
        "INSERT INTO activities (id, title, date, description, type, leaderId) VALUES (?, ?, ?, ?, ?, ?)"
      ).bind(generateUUID(), activity.title, activity.date, activity.description, "Event", id));
    ...
    console.error(`Error updating state leader ${id}:`, e);
    return c.json({ error: e.message }, 500);
  }
});
stateLeadersRouter.delete("/:id", jwtMiddleware, async (c) => {
  const id = c.req.param("id");
  try {
    await c.env.DB.batch([
      c.env.DB.prepare("DELETE FROM state_leaders WHERE id = ?").bind(id),
      c.env.DB.prepare("DELETE FROM activities WHERE leaderId = ?").bind(id),
      c.env.DB.prepare("DELETE FROM milestones WHERE leaderId = ?").bind(id)
    ]);
    return new Response(null, { status: 204 });
  } catch (e) {
    console.error(`Error deleting state leader ${id}:`, e);
    return c.json({ error: e.message }, 500);
  }
});
api.route("/state_leaders", stateLeadersRouter);
createCrudEndpoints(api, "milestones");
...
// .wrangler/tmp/bundle-KIn8rP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KIn8rP/middleware-loader.entry.ts
