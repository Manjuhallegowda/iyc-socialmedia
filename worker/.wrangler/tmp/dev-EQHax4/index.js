...
// .wrangler/tmp/bundle-oTORUF/checked-fetch.js
...
auth.post("/login", async (c) => {
  try {
    ...
    if (userCount === 0) {
      ...
      const fetched = await c.env.DB.prepare("SELECT * FROM users WHERE username = ?").bind(username).first();
      if (!fetched) {
        return c.json({ error: "Invalid credentials" }, 401);
      }
      user = fetched;
      const hashed = await hashPassword(password, user.salt, user.iterations || 15e4);
...
auth.post("/change-password", jwtMiddleware, async (c) => {
  try {
    ...
    const currentHashed = await hashPassword(currentPassword, user.salt, user.iterations || 15e4);
    ...
    await c.env.DB.prepare("UPDATE users SET hashedPassword = ?, salt = ?, hashVersion = ?, iterations = ?, algorithm = ? WHERE id = ?").bind(newHashed.hashHex, newSalt, newHashed.version, newHashed.iterations, newHashed.algorithm, userId).run();
...
var createCrudEndpoints = /* @__PURE__ */ __name((router, tableName, options = {}) => {
  ...
  router.get(`${basePath}/:id`, async (c) => {
    ...
    try {
      const item = await c.env.DB.prepare(`SELECT * FROM ${tableName} WHERE id = ?`).bind(id).first();
...
// C:/Users/Manju/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/Manju/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-oTORUF/middleware-insertion-facade.js
...
// C:/Users/Manju/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-oTORUF/middleware-loader.entry.ts
