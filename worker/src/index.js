...
const hashPassword = async (password, saltBase64, iterations = 150_000) => {
    ...
    const derivedBits = await crypto.subtle.deriveBits({
        ...
        iterations,
        hash: 'SHA-256',
    }, passwordKey, 256 // 256 bits = 32 bytes
    );
...
auth.post('/login', async (c) => {
    try {
        ...
        const userCountRow = await c.env.DB.prepare('SELECT COUNT(*) AS count FROM users').first();
        ...
        if (userCount === 0) {
            ...
            await c.env.DB.prepare('INSERT INTO users (id, username, hashedPassword, salt, hashVersion, iterations, algorithm) VALUES (?, ?, ?, ?, ?, ?, ?)')
                .bind(id, username, hashedPassword, salt, hashed.version, hashed.iterations, hashed.algorithm)
                .run();
            console.log('Admin user created successfully.');
        }
        else {
            // Normal authentication flow
            const fetched = await c.env.DB.prepare('SELECT * FROM users WHERE username = ?')
                .bind(username)
                .first();
            if (!fetched) {
...
users.post('/', async (c) => {
    try {
        ...
        const hashed = await hashPassword(password, salt);
        await c.env.DB.prepare('INSERT INTO users (id, username, hashedPassword, salt, hashVersion, iterations, algorithm) VALUES (?, ?, ?, ?, ?, ?, ?)')
            .bind(id, username, hashed.hashHex, salt, hashed.version, hashed.iterations, hashed.algorithm)
            .run();
...
const createCrudEndpoints = (router, tableName, options = {}) => {
    const basePath = `/${tableName}`;
    const transformAfterFetch = (item) => {
        return options.afterFetch ? options.afterFetch(item) : item;
    };
    const transformBeforeSave = (item) => {
    ...
    router.get(`${basePath}/:id`, async (c) => {
        ...
        try {
            const item = await c.env.DB.prepare(`SELECT * FROM ${tableName} WHERE id = ?`)
                .bind(id)
                .first();
            if (!item)
    ...
    router.post(basePath, jwtMiddleware, async (c) => {
        try {
            ...
            const keys = Object.keys(newItem).filter((k) => typeof newItem[k] !== 'undefined');
            const placeholders = keys.map(() => '?').join(', ');
            const values = keys.map((k) => newItem[k]);
            const fields = keys.join(', ');
            await c.env.DB.prepare(`INSERT INTO ${tableName} (${fields}) VALUES (${placeholders})`)
                .bind(...values)
                .run();
    ...
    router.put(`${basePath}/:id`, jwtMiddleware, async (c) => {
        ...
        try {
            ...
            const stmnt = await c.env.DB.prepare(`UPDATE ${tableName} SET ${fields} WHERE id = ?`)
                .bind(...values, id)
                .run();
    ...
    router.delete(`${basePath}/:id`, jwtMiddleware, async (c) => {
        const id = c.req.param('id');
        try {
            const stmnt = await c.env.DB.prepare(`DELETE FROM ${tableName} WHERE id = ?`)
                .bind(id)
                .run();
            if (!stmnt || stmnt.meta?.changes === 0) {
                return c.json({ error: 'Not Found' }, 404);
...
const kpyccTeamTransforms = {
    beforeSave: (member) => ({
        ...member,
        social: JSON.stringify(member.social || {}),
        activity: JSON.stringify((member.activity || '').split(',').map((s) => s.trim())),
        mailstone: JSON.stringify((member.mailstone || '').split(',').map((s) => s.trim())),
    }),
    afterFetch: (member) => {
        let social = {};
        try {
...
const socialMediaTeamTransforms = {
    beforeSave: (member) => ({
        ...member,
        socialMedia: JSON.stringify(member.socialMedia || {}),
    }),
    afterFetch: (member) => ({
        ...member,
...
createCrudEndpoints(api, 'state_leaders', stateLeaderTransforms);
createCrudEndpoints(api, 'social_media_team', socialMediaTeamTransforms);
createCrudEndpoints(api, 'legal_team');
...
app.get('/map-data/karnataka-districts.json', async (c) => {
    try {
        const response = await fetch('https://raw.githubusercontent.com/udit-001/india-maps-data/main/states/karnataka/topojson/karnataka.json');
        if (!response.ok) {
            return c.json({ error: `Failed to fetch TopoJSON: ${response.statusText}` }, response.status);
        }
