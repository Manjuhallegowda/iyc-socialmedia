...
const LeaderModal: React.FC<LeaderModalProps> = ({ leader, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header / Banner */}
        <div className="h-32 bg-gradient-to-r from-saffron via-white to-indiaGreen opacity-90 relative">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 bg-white/50 hover:bg-white text-indiaGreen p-2 rounded-full transition-colors"
          >
            <X size={24} />
          </button>
        </div>

        <div className="px-6 sm:px-8 pb-8 -mt-6 relative z-10">
          {/* Profile Header */}
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 mb-6">
            ...
            <div className="text-center sm:text-left mb-2">
              <h2 className="text-2xl font-bold text-indiaGreen">
                {leader.name}
              </h2>
              <p className="text-saffron font-semibold">{leader.designation}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-bold text-indiaGreen border-b-2 border-saffron/20 pb-2 mb-3">
                  Basic Info
                </h3>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {leader.protests.map((p, idx) => (
                      <div
                        key={idx}
                        className="bg-gray-50 p-3 rounded-md border-l-4 border-indiaGreen"
                      >
